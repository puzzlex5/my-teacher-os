(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.TeacherOSAutonomous40=api})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function actionFor(entity,now){const n=dueDays(entity?.due,now),d=domain(entity?.title),score=baseScore(entity,now),sev=severity(score,n,!!entity?.dueConflict);let reason='여러 업무 정보를 종합해 우선순위를 계산했습니다.',next='관련 원본을 확인하고 다음 처리 단계를 정리하세요.',safe=false,task='',phase='watch';
    if(entity?.dueConflict){reason='연결된 원본들 사이에 마감일 차이가 있어 사람이 확인해야 합니다.';next='실제 기준 마감일을 확인하고 충돌을 해소하세요.';phase='conflict'}
    else if(n!==null&&n<0){reason=`마감이 ${Math.abs(n)}일 지났지만 완료로 확인되지 않았습니다.`;next='완료 여부를 확인하고 미완료면 즉시 처리하세요.';safe=true;task=`[자동 우선] ${entity.title} · 마감 경과 확인`;phase='overdue'}
    else if(n===0){reason='오늘 마감 업무입니다.';next='오늘 안에 제출·입력·확인을 완료하세요.';safe=true;task=`[오늘 최우선] ${entity.title}`;phase='today'}
    else if(n!==null&&n<=2){reason=`마감까지 ${n}일 남았습니다.`;next=d==='assessment'?'평가 준비물·안내·입력 상태를 최종 점검하세요.':'제출물·첨부·결재 상태를 최종 점검하세요.';safe=true;task=`[D-${n}] ${entity.title} 최종 점검`;phase='soon'}
    else if(n!==null&&n<=7){reason=`이번 주 안에 마감되는 업무입니다.`;next='이번 주 처리 순서에 포함하고 필요한 선행작업을 확인하세요.';safe=true;task=`[이번 주] ${entity.title} 준비`;phase='week'}
    else if(!entity?.due&&Number(entity?.sourceCount)>=2){reason='여러 시스템에서 같은 업무가 감지됐지만 기준 마감일이 없습니다.';next='공식 마감일을 한 번 확인하세요.';safe=false;phase='missing_due'}
    return{id:`v40:${entity?.id||Math.random().toString(36).slice(2)}`,entityId:entity?.id||'',title:txt(entity?.title)||'통합 업무',due:txt(entity?.due),domain:d,severity:sev,score,reason,nextAction:next,autoSafe:safe&&!entity?.dueConflict,taskText:task,phase,sourceCount:Number(entity?.sourceCount)||0,dueConflict:!!entity?.dueConflict};
  }
  function healthActions(ctx={}){const out=[];const add=(id,title,why,score)=>out.push({id:`v40:health:${id}`,entityId:'',title,domain:'system',severity:score>=100?'critical':'warning',score,reason:why,nextAction:'연결 상태를 확인하고 자동 재시도를 기다리거나 설정을 점검하세요.',autoSafe:false,taskText:'',phase:'health',sourceCount:0,dueConflict:false});
  ...
  function safeTaskCandidates(plan,tasks,max=4){const seen=existingAgentKeys(tasks),out=[];for(const a of arr(plan?.actions)){if(out.length>=max)break;if(!a.autoSafe||!a.taskText)continue;const key=`${a.id}:${a.phase}`;if(seen.has(key))continue;out.push({agentKey:key,text:a.taskText,due:a.due||'',done:false,agentGenerated:true,agentCategory:'자율업무',autonomous40:true,sourceEntityId:a.entityId,priority:a.severity==='critical'?'high':'normal',createdAt:new Date().toISOString()});seen.add(key)}return out}
