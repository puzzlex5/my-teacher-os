(function(){
  ...
  function resultAge(r){const t=r?.checkedAt?new Date(r.checkedAt).getTime():NaN;return Number.isFinite(t)?Date.now()-t:Infinity}
  function freshPass(r){const age=resultAge(r);return r?.ok===true&&age>=0&&age<=DAY}
  ...
  function render(){
    ...
    else if(r.ok){box.className='mini pending';box.textContent=`이전 검사는 통과했지만 24시간 유효기간이 지났습니다. 최신 검사가 끝날 때까지 로컬 원본 보관을 잠급니다. · ${when}`}
