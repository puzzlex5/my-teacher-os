# Teacher OS Desktop Bridge v0.45
...
v0.45부터는 별도의 **로컬 watchdog**이 Bridge 상태를 확인합니다. Bridge 프로세스가 비정상 종료되거나 응답하지 않으면 사용자에게 버튼을 요구하지 않고 자동으로 다시 시작합니다.
...
설치 프로그램은 기존 Bridge가 실행 중이면 교체 후 v0.45 watchdog으로 다시 시작합니다. 관리자 권한, 업무포털 로그인정보, NEIS/K-에듀파인 비밀번호를 요구하지 않습니다.
...
- Bridge가 죽으면 자동으로 `bridge_v44.py`를 다시 실행합니다.
