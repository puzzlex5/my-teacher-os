...
$bridge = Join-Path $here 'bridge_v37.py'
if (-not (Test-Path $bridge)) { throw 'bridge_v37.py 파일이 없습니다.' }
...
Write-Host 'Desktop Bridge v0.37을 지금 시작합니다.'
...
if (Test-Path $config) {
  ...
  Write-Host ''
  Write-Host 'NEIS 내보내기 파일은 학생 이름/원문을 보내지 않고 미입력·누락 건수만 집계합니다.' -ForegroundColor Green
} else {
  Write-Warning '설정파일 생성 확인이 필요합니다. bridge_v37.py를 직접 한 번 실행하세요.'
}
