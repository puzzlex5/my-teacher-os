...
const yUnknown={timetable:[{day:'월',period:2,target:'2-3',subject:'음악',time:''}],liveTimetableWeeks:{},classProgress:{}};
const cUnknown=D.lessonContext(yUnknown,now);
ok(cUnknown.status==='unknown','missing school level must not assume middle-school bell times');
ok(cUnknown.slot===null&&cUnknown.reason==='bell-time-unconfirmed','unconfirmed bell time must fail closed without a lesson conclusion');
ok(D.periodRange(yUnknown,2)===null,'period range must not invent a school-level default when school level is unknown');
const yUnknownExact={...yUnknown,timetable:[{day:'월',period:2,target:'2-3',subject:'음악',time:'10:05~10:50'}]};
const cUnknownExact=D.lessonContext(yUnknownExact,new Date('2026-08-24T10:00:00'));
ok(cUnknownExact.status==='next'&&cUnknownExact.exact===true&&cUnknownExact.seconds===300,'explicit bell time must remain usable without school-level metadata');
...
['teacherDesk27','다음 교시','기본 교시시간 기준(예상)','교시 시간 미확인','학교급/교시 시각 확인 필요','myTeacherOS.staffContacts.v1','연락처는 이 브라우저에만 저장','메시지·공문 빠른 정리','없는 내용을 만들어내지 않습니다','위젯 설정'].forEach(t=>ok(app.includes(t),'app token missing: '+t));
...
ok(core.includes("status:'unknown'")&&core.includes("reason:'bell-time-unconfirmed'"),'Teacher Desk core must preserve unknown bell-time state');
