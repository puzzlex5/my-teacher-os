...
async function openApp(browser,url,viewport){
  ...
  await page.waitForFunction(()=>{
    ...
    return nav.length>=12 && document.querySelector('#worklibrary') && document.querySelector('#teacherDesk');
  },null,{timeout:20000});
  // Let async library/config reads settle without requiring network-idle (OCR/CDN assets
  // may legitimately keep connections alive).
...
for(const device of [
  ...
  test(`v1 bundled runtime matches legacy DOM and navigation on ${device.name}`,async({browser})=>{
    ...
    try{
      ...
      const legacyFingerprint=await fingerprint(legacy.page);
      const bundleFingerprint=await fingerprint(bundle.page);
      expect(bundleFingerprint).toEqual(legacyFingerprint);

      const legacyNav=await navigationFingerprint(legacy.page);
      const bundleNav=await navigationFingerprint(bundle.page);
      expect(bundleNav).toEqual(legacyNav);
    } finally {
