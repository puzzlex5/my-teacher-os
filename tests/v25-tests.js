...
const goodEvidence=[
  {id:'e1',date:'2026-04-12',eligible:true,area:'subject',kind:'교과관찰',text:'모둠 합주에서 리듬이 맞지 않는 부분을 스스로 찾아 반복 연습하고 친구에게 박자를 설명함.'},
  {id:'e2',date:'2026-05-03',eligible:true,area:'subject',kind:'교과관찰',text:'합주 발표에서 다른 파트의 소리를 들으며 자신의 연주 세기를 조절하고 모둠원과 시작 박자를 맞춤.'}
];
...
const unspecifiedEligibility=R.analyzeDraft({area:'subject',evidence:[{id:'u1',area:'subject',kind:'교과관찰',text:'합주에서 반복 연습하고 박자를 설명함.'}],text:'합주에서 반복 연습하고 박자를 설명함.'});
if(!unspecifiedEligibility.critical||unspecifiedEligibility.evidenceCount!==0||!unspecifiedEligibility.issues.some(x=>x.code==='NO_EVIDENCE'))throw new Error('evidence without explicit eligible=true must fail closed');
...
console.log('v0.25 grounded school-record quality tests passed with explicit eligibility, strict sentence, Korean particle, stale-evidence, direct-PII, and stale-snapshot checks');
