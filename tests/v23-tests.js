...
const privacyMisclassified=D.fuseSuggestion({baseConfidence:.98,docClass:'admin',docConfidence:.98,extractionQuality:.95,kind:'admin',sensitive:true});
assert.equal(privacyMisclassified.auto,false,'privacy signals must fail closed even when a sensitive document is misclassified as a normal admin document');
assert.ok(privacyMisclassified.confidence<=.58,'privacy-sensitive candidates must be capped below auto-apply confidence');
...
['PaddleOCR','PP-OCRv5','pdf-hybrid-layout+ocr','feedbackSignature','v23DocReport','고정밀 다시 분석','학생자료는 자동 적용 차단','TeacherOSStorage.writeJSON(KEY,state)','sensitive=privacy.length>0',"sensitive:classId==='student'||(report.privacy||[]).length>0",'개인정보 형태가 감지된 파일은 문서 분류와 관계없이 자동 적용을 막았습니다.'].forEach(t=>{if(!app.includes(t))throw new Error('v0.23 app missing: '+t)});
if(app.includes("sensitive=privacy.length>0&&doc.classId==='student'"))throw new Error('v0.23 privacy auto-apply guard still depends on student classification');
...
console.log('v0.23 document intelligence tests passed with shared-storage and privacy fail-closed guards');
