...
for(const token of ['TeacherOSLifecycle','lifecycle22.onRender','fallbackRender22','ensureUI();wrapJSONImport();renderRecovery()']){
  if(!app.includes(token))throw new Error(`prepared v22 missing ${token}`);
}
fs.writeFileSync(appPath,app,'utf8');
console.log('Prepared v1 v22 local recovery to use the shared render lifecycle with isolated-test fallback.');
