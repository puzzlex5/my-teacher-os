...
const replacement="const d=D.lessonContext(y,new Date()),truth=truth27(y,d);if(truth&&!truth.known){box.innerHTML=`<div class=\"desk-next-top27\"><span class=\"desk-status27\">${esc(truth.label||'시간표 미확인')}</span><span class=\"mini\">확정 가능한 시간표 없음</span></div><h3>오늘 수업 여부를 확정할 수 없습니다.</h3><div class=\"desk-progress27\"><span>${truth.reason==='collector-error'?'컴시간 수집기가 최신표를 가져오지 못했습니다. 이전 표를 현재표로 간주하지 않습니다.':truth.reason==='transport-unavailable'?'컴시간 동기화 경로가 구성되지 않았습니다. 이전 표를 현재표로 간주하지 않습니다.':'컴시간 실제표와 기본 시간표가 모두 확인되지 않았습니다. 0건으로 처리하지 않습니다.'}</span></div><div class=\"desk-next-foot27\"><button type=\"button\" class=\"btn secondary tiny\" data-go27=\"timetable\">시간표 확인</button></div>`;return}if(d.status==='unknown'){box.innerHTML=`<div class=\"desk-next-top27\"><span class=\"desk-status27\">교시 시간 미확인</span><span class=\"mini\">${esc(d.source)} · 학교급/교시 시각 확인 필요</span></div><h3>수업은 등록되어 있지만 현재·다음 교시를 확정할 수 없습니다.</h3><div class=\"desk-progress27\"><span>학교급 또는 교시 시작·종료 시각을 확인하면 정확한 현재/다음 수업을 표시합니다.</span></div><div class=\"desk-next-foot27\"><button type=\"button\" class=\"btn secondary tiny\" data-go27=\"timetable\">시간표 확인</button></div>`;return}if(!d.slot){box.innerHTML=`<div class=\"desk-next-top27\"><span class=\"desk-status27\">${d.status==='done'?'오늘 수업 완료':'다음 수업 없음'}</span><span class=\"mini\">${esc(d.source)}</span></div><h3>${d.status==='done'?'오늘 예정된 수업이 끝났습니다.':'오늘 등록된 수업이 없습니다.'}</h3><div class=\"desk-next-foot27\"><button type=\"button\" class=\"btn secondary tiny\" data-go27=\"timetable\">시간표 확인</button></div>`;return}";
...
else if(!src.includes("if(d.status==='unknown')")){
  const truthEnd="</div>`;return}if(!d.slot){";
  if(!src.includes(truthEnd))throw new Error('v1 v27 bell-time truth preparation failed: no-slot anchor missing');
  src=src.replace(truthEnd,"</div>`;return}if(d.status==='unknown'){box.innerHTML=`<div class=\"desk-next-top27\"><span class=\"desk-status27\">교시 시간 미확인</span><span class=\"mini\">${esc(d.source)} · 학교급/교시 시각 확인 필요</span></div><h3>수업은 등록되어 있지만 현재·다음 교시를 확정할 수 없습니다.</h3><div class=\"desk-progress27\"><span>학교급 또는 교시 시작·종료 시각을 확인하면 정확한 현재/다음 수업을 표시합니다.</span></div><div class=\"desk-next-foot27\"><button type=\"button\" class=\"btn secondary tiny\" data-go27=\"timetable\">시간표 확인</button></div>`;return}if(!d.slot){");
}
...
console.log('Prepared v1 Teacher Desk truth guard, bell-time uncertainty, and shared lifecycle refresh.');
