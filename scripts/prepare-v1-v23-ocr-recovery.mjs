...
const next=`  async function getPaddle23(){
    ...
    paddlePromise23=pending.catch(err=>{if(paddlePromise23===pending||paddlePromise23) paddlePromise23=null;throw err});
    return paddlePromise23;
  }`;

if(src.includes(old)) src=src.replace(old,next);
else if(!src.includes('paddlePromise23=pending.catch(err=>')) throw new Error('v1 v23 OCR recovery preparation failed: expected getPaddle23 source not found');

for(const token of [
  'const pending=(async()=>',
  'paddlePromise23=pending.catch(err=>',
  'paddlePromise23=null;throw err'
]) if(!src.includes(token)) throw new Error(`v1 prepared v23 OCR recovery missing: ${token}`);

fs.writeFileSync(path,src,'utf8');
console.log('Prepared v1 app-v23 so a transient PaddleOCR initialization failure does not poison the entire browser session.');
