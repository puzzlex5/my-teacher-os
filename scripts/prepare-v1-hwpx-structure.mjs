...
const helperSource=fs.readFileSync('scripts/v1-hwpx-structure-core.mjs','utf8').replace(/export\s+function\s+hwpxXmlToText/,'function hwpxXmlToText').trim();
...
if(!src.includes('function hwpxXmlToText(xml)')){
  ...
  src=src.replace(marker,helperSource+'\n'+marker);
}
...
for(const token of ['function hwpxXmlToText(xml)','colSpan','tables.push(tableText(table))','sort((a,b)=>Number((a.match(/section(\\d+)/i)','return s.trim()']){
